...
export async function Header() {
    ...
    return (
        <header className="header absolute left-0 right-0 top-0 z-50 ~h32/48 ~px-4/6 ~py-4/6 hd:h-32">
                    ...
                    <ul className="flex flex-wrap items-center justify-center gap-8">
                        {settings.data.navigation.map((item) => (
                            <li key={item.link.text}>
                                <PrismicLink field={item.link} className="~text-lg/xl">{item.link.text}</PrismicLink>
                            </li>
                        ))}
                    </ul>
                </nav>
